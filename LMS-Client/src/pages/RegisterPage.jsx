...
// Updated icons for a more futuristic feel
...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Assuming signUp now accepts all registration fields
      await signUp(email, password, fullName, role);
      toast.success('Account created successfully! Preparing your workspace...');
      // Update navigation to match the original goal
      navigate('/bank-setup'); 
    } catch (error) {
  ...
  // Define subtle animation variants for a staggered, smooth entry
  ...
  // Custom component for the role buttons to simplify JSX and apply consistent styling
    ...
    <motion.button
      ...
      className={`p-4 rounded-xl border-2 transition-all duration-300 ${
        currentRole === targetRole
          ? 'border-cyan-500 bg-cyan-900/40 shadow-xl shadow-cyan-500/20' // Active state with glow
          : 'border-gray-700 bg-gray-800/50 hover:border-cyan-500/50' // Inactive state
  ...
  return (
    // Background: Dark, dynamic gradient for a futuristic/cyberpunk feel
    <div className="min-h-screen bg-gray-900 flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="text-center mb-10">
          ...
          >
            <Shield className="w-8 h-8 text-gray-900" /> {/* Shield icon for security/account creation */}
          </motion.div>
        ...
        {/* Register Form Container - Glassmorphism Style */}
        <motion.div
          variants={itemVariants}
